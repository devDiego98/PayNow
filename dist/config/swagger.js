...
exports.getSwaggerJSDocOptions = getSwaggerJSDocOptions;
exports.setupSwagger = setupSwagger;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const swagger_jsdoc_1 = __importDefault(require("swagger-jsdoc"));
const swagger_ui_express_1 = __importDefault(require("swagger-ui-express"));
function getSwaggerJSDocOptions() {
    ...
    return {
        definition: {
            ...
            info: {
                ...
                version: "1.0.0",
                description: "Unified Payment Gateway Aggregation API. Integrate Stripe, PayPal, and Mercado Pago through a single normalized interface.",
            ...
            components: {
                ...
                schemas: {
                    CreatePaymentRequest: {
                        ...
                        description: "Provide exactly one of paymentMethod (new token/pm/tok) or savedCardId (from GET /api/v1/cards).",
                        properties: {
                            provider: {
                                type: "string",
                            ...
                            savedCardId: {
                                type: "string",
                                description: "Id from POST /api/v1/cards — pay with that saved card (Stripe only; customer.email must match).",
                            },
                            metadata: {
                                type: "object",
                    ...
                    MercadoPagoPaymentLinkRequest: {
                        ...
                        properties: {
                            amount: {
                                type: "integer",
                                description: "Amount in smallest currency unit (e.g. centavos for ARS), same as POST /api/v1/payments",
                                example: 100050,
                            },
                            currency: {
                                type: "string",
                            ...
                            defaultInstallments: {
                                ...
                                minimum: 1,
                                maximum: 36,
                                description: "Preselected installment count (`payment_methods.default_installments`). Must be ≤ maxInstallments.",
                            },
                        },
                    },
                    ...
                    MercadoPagoSubscriptionLinkRequest: {
                        ...
                        properties: {
                            amount: {
                                type: "integer",
                                description: "Amount in smallest currency unit (e.g. ARS centavos), converted to Mercado Pago `transaction_amount`",
                                example: 999900,
                            },
        ...
        apis: [
            path_1.default.join(projectRoot, "src", "controllers", "**", "*.ts"),
            path_1.default.join(projectRoot, "src", "routes", "**", "*.ts"),
        ],
    };
}
/**
 * Prefer a generated OpenAPI snapshot (`npm run build` writes `dist/openapi.json`) so Swagger UI
 * matches the built spec in dev and prod. Falls back to swagger-jsdoc scanning `src` routes.
 *
 * Note: when running via ts-node, `__dirname` is `src/config`, so we must look at
 * `<projectRoot>/dist/openapi.json`, not only `<__dirname>/../openapi.json` (which would be `src/openapi.json`).
 */
function loadSwaggerSpec() {
    const projectRoot = path_1.default.join(__dirname, "..", "..");
    const snapshotCandidates = [
        path_1.default.join(projectRoot, "dist", "openapi.json"),
        path_1.default.join(__dirname, "..", "openapi.json"),
    ];
    ...
    return (0, swagger_jsdoc_1.default)(getSwaggerJSDocOptions());
}
 ...
 */
function swaggerUiOnComplete() {
    /* eslint-disable @typescript-eslint/no-explicit-any -- DOM / browser-only; TS lib is ES2022 without DOM */
    const w = globalThis;
    const sessionKey = typeof globalThis.crypto !== "undefined" && globalThis.crypto.randomUUID
    ...
    function isIdempotencyField(inp) {
        const ph = inp.getAttribute("placeholder") ?? "";
        const nm = inp.getAttribute("name") ?? "";
        if (/idempotency/i.test(ph) || /idempotency/i.test(nm))
            return true;
        const row = inp.closest("tr") ?? inp.closest("[class*='parameter']") ?? inp.closest(".opblock");
        if (row && /idempotency/i.test(row.textContent ?? ""))
            return true;
        return false;
    }
    function fill() {
        ...
        for (let i = 0; i < inputs.length; i++) {
            ...
            if (!isIdempotencyField(inp))
                continue;
            if (inp.dataset.paygateIdempotency === sessionKey)
                continue;
...
function swaggerRequestInterceptor(request) {
    ...
    if (typeof Headers !== "undefined" && request.headers instanceof Headers) {
        ...
        request.headers["Idempotency-Key"] = id;
    }
    return request;
}
const swaggerUiOpts = {
...
function setupSwagger(app) {
    app.use("/api/docs", swagger_ui_express_1.default.serve, swagger_ui_express_1.default.setup(exports.swaggerSpec, swaggerUiOpts));
}
